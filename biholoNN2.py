...
class Biholomorphic(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        zzbar = tf.reshape(zzbar, [-1, tf.shape(zzbar)[1]**2])
...
class Biholomorphic_k2(keras.layers.Layer):
    '''A layer transform zi to symmetrized zi1*zi2'''
    def __init__(self):
    ...
    def call(self, inputs):
        ...
        zz = tf.reshape(zz, [-1, tf.shape(zz)[1]**2])
        return remove_zero_entries(zz)

class Biholomorphic_k3(keras.layers.Layer):
    '''A layer transform zi to symmetrized zi1*zi2*zi3'''
    def __init__(self):
	...
	zz = tf.transpose(zz, perm=[0, 2, 3, 1])
        zz = tf.reshape(zz, [-1, tf.shape(zz)[1]**3]) 
        return remove_zero_entries(zz)
